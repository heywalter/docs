...
Tools 是模型可以直接调用的函数或指令，用于与结构化数据进行交互，例如查询数据模型、执行 MongoDB 查询，或在用户确认后创建连接和任务。这些工具会通过 MCP 自动发现并生效，无需额外配置。

| 工具名称                  | 功能说明                                                         |
| ------------------------- | ---------------------------------------------------------------- |
| `listConnection`          | 列出 TapData 中可用的数据库连接                                  |
| `listDataModel`           | 获取连接下的数据模型结构，可选择是否返回字段和索引信息           |
| `createConnection`        | 创建数据库连接；如未传入配置，可先返回该数据源所需的连接配置字段 |
| `createMigrateTask`       | 创建数据复制任务，支持按指定表或表名表达式选择源表               |
| `createMergeTableTask`    | 创建主从合并任务，将子表数据合并到目标表或文档中                 |
| `startTask`               | 启动一个或多个 TapData 任务；编辑状态的任务会先确认再启动        |
| `updateFieldDescription`  | 批量更新表模型中字段的业务描述                                   |
| `listCollection`          | 列出 MongoDB 数据库中的集合                                      |
| `query`                   | 执行 MongoDB 查询                                                |
| `count`                   | 统计符合条件的 MongoDB 文档数量                                  |
| `aggregate`               | 使用 MongoDB 聚合管道处理数据                                    |

其中，`createMigrateTask` 用于创建数据复制任务，通常需要提供任务名称、源/目标连接 ID、源/目标数据库类型和表选择方式；当表选择方式为指定表时，可通过 `tableNames` 传入表名列表，当表选择方式为表达式时，可通过 `tableExpression` 传入表名匹配表达式。任务创建后，如需立即运行，可继续调用 `startTask`。
...
当前 MCP 服务已支持上下文数据读取，以及连接创建、任务创建、任务启动等操作类工具。操作类工具会对 TapData 平台配置或任务状态产生影响，建议在 AI Agent 执行前明确确认连接、表范围、同步类型等关键参数。
