...
- [StarRocks](../prerequisites/warehouses-and-lake/starrocks.md) 作为源库时支持全量同步视图，可在选择同步对象时同时选择表和视图

### 功能优化

- 优化[部署](../administration/production-deploy/install-tapdata-ha.md)阶段的 `./tapdata sid` 命令，首次启动前无需预先填写启动参数即可获取申请 License 所需的 SID 信息
- 优化[任务监控](../user-guide/copy-data/monitor-task.md)中节点及节点详情的全量同步进度展示，提升进度统计的准确性
- 优化任务里程碑与运行记录展示，支持查看历史运行的全量同步耗时，并使跳过全量阶段的任务按已完成状态展示
- 优化源节点自定义条件中的日期字段配置，支持在具体日期和相对日期之间切换
- 优化共享挖掘集合查询性能，减少特定场景下的慢查询
- 升级部分第三方依赖，修复已知安全风险

### 缺陷修复

- 修复 API Server 在特定环境下可能启动失败或产生兼容性告警的问题
- 修复 API Server 在 JSON Query、自定义查询和 GraphQL 等场景下，部分过滤条件、必填参数校验或错误提示未按预期生效的问题，完善 OR、LIKE、IN 等查询条件适配
- 修复任务停止、重置或重新启动后，监控指标可能为负值、增量时间不可见或任务直接进入错误状态的问题
- 修复开启共享挖掘后目标表列表可能无法加载，以及子账号访问已授权数据源的共享挖掘任务时可能报错的问题
- 修复[数据校验](../user-guide/verify-data.md)页面中校验连接无法加载或任务下拉列表无法选择的问题
- 修复 Oracle 补充日志未启用或状态变化后，系统未能按预期提示的问题
- 修复 Db2 for i 作为源端使用自定义查询时，时间字段可能出现时区偏差的问题
- 修复 SQL Server 同步场景下，启用多线程写入并保留原表结构时日期字段转换可能报错，以及 SQL Server 间全量同步可能出现 SQL 语法错误的问题
- 修复以 Excel 作为源端的数据转换任务长时间运行后，源端数据变更可能未同步至目标端的问题

## 4.21.0

### 新增功能

- [PostgreSQL](../prerequisites/on-prem-databases/postgresql.md) 作为源库时支持采集 DDL 事件，可捕获新增字段、字段改名、字段属性变更和删除字段等结构变更，并与目标端 DDL 应用能力配合完成结构同步

### 功能优化

- [MCP Server](../mcp/quick-start.md) 完成模块重构与升级，提升协议适配与工具扩展能力
- 优化前端多语言消息适配，支持 i18nMessage 统一处理
- 优化任务监控指标展示，可根据任务 CPU、内存指标统计开关控制相关图表展示
- 优化[通知与告警](../user-guide/other-settings/notification.md)中的邮件模板变量，支持在通知内容中引用任务描述
- 优化任务画布节点位置居中逻辑，提升画布编排体验

### 缺陷修复

- 修复 JS 处理节点在特定场景下脚本执行隔离不够严格的问题，提升自定义脚本运行安全性
- 修复部分敏感信息展示或存储保护不充分的问题，提升凭据安全性
- 修复合并任务在共享挖掘不可用并回退普通 CDC 时，源连接进入增量阶段可能报错的问题
- 修复任务全量完成或重启后，任务指标、心跳状态或增量延迟告警可能异常的问题
- 修复新增或修改 API 时路径重复校验未按预期生效，以及发布 API 在特定场景下可能失败的问题
- 修复创建角色时密码规则限制提示缺失的问题
- 修复 CICD 任务导入/发布流程中，可能出现项目重复创建、主从任务运行失败或 FDM 新增表后触发非预期全量的问题
- 修复单节点部署重启后，任务可能长时间停留在“启动中”的问题
- 修复主从合并任务中，子表内嵌文档删除字段后目标端未按预期同步删除的问题
- 修复任务重试告警恢复后仍显示的问题
- 修复 PostgreSQL 同步至 Sybase 的反向同步场景中，任务可能因外键约束校验失败的问题
- 修复轮询请求持续刷新用户活跃时长，导致无用户操作时会话仍可能保持活跃的问题
- 修复子账号无法查看引擎标签的问题
- 修复启用 DDL 事件采集后，任务启动阶段可能报错的问题
