...
| 二进制   | BINARY、FILE                                  | -                        |
| 布尔     | BOOLEAN                                       | -                        |
| 日期/时间 | DATE、TIME、TIMESTAMP_NTZ、TIMESTAMP_TZ、TIMESTAMP_LTZ | TIMESTAMP_LTZ 仅用于查询侧识别 |
...
3. 为刚创建的账号授予权限，您也可以基于业务需求设置更精细化的权限控制。
   ...
   如果使用混合表，创建权限请以 Snowflake 账号当前权限模型为准：部分账号使用 `CREATE TABLE` 权限控制混合表创建；启用 Snowflake 2026_02 行为变更后，可能需要单独授予 `CREATE HYBRID TABLE` 权限。如果使用动态表，创建动态表的角色还需要对动态表查询中引用的基表或视图具备查询权限，并可使用指定的 Warehouse。若需要对已有目标表执行字段级 DDL 变更，请确保同步角色具备修改该表结构所需的权限。
